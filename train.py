...
from gaussian_renderer import render_surfel, render_initial, render_volume, render_surfel_textured, network_gui
import sys
from scene import Scene, GaussianModel
from scene.gaussian_model import TexturedGaussianModel
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, model_path, debug_from=None):
    ...
    # Stage ordering: init -> volume -> surfel(forward PBR) -> textured(deferred).
    # Setting texture_from_iter beyond `iterations` yields a pure baseline run.
    assert opt.init_until_iter <= opt.volume_render_until_iter < opt.texture_from_iter, \
        "Require init_until_iter <= volume_render_until_iter < texture_from_iter"
    ...
    # textured_stage: True once optimisation has switched to the per-primitive
    # texture model (4th stage). It can also be entered directly via checkpoint.
    textured_stage = False
    if checkpoint:
        (model_params, first_iter) = torch.load(checkpoint)
        # capture() element 2 is the metallic tensor: 4D => textured model
        # (per-primitive texture maps), 2D => baseline model (per-primitive scalar).
        is_textured_ckpt = isinstance(model_params, (tuple, list)) and model_params[2].dim() == 4
        if is_textured_ckpt:
            # Textured checkpoint -> resume the texture stage.
            print(f"\n[CKPT] Textured checkpoint ({first_iter} it); resuming the texture stage.")
            gaussians = TexturedGaussianModel(dataset.sh_degree, texture_size=opt.texture_size)
            gaussians.restore(model_params, opt, checkpoint, dataset)
            gaussians.freeze_geometry()
            scene.gaussians = gaussians
            textured_stage = True
        elif first_iter >= opt.texture_from_iter:
            # Baseline checkpoint at/after the texture boundary -> bootstrap the texture stage.
            print(f"\n[CKPT] Baseline checkpoint ({first_iter} it); bootstrapping the texture stage.")
            gaussians = TexturedGaussianModel(dataset.sh_degree, texture_size=opt.texture_size)
            gaussians.restore_from_pretrained(model_params, opt, checkpoint, dataset)
            gaussians.freeze_geometry()
            scene.gaussians = gaussians
            textured_stage = True
        else:
            # Baseline checkpoint before the texture boundary -> resume baseline training.
            print(f"\n[CKPT] Baseline checkpoint ({first_iter} it); resuming baseline training.")
            gaussians.restore(model_params, opt)

    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]
    background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")
    gaussExtractor = GaussianExtractor(gaussians, render_initial, pipe, bg_color=bg_color)
    if textured_stage and gaussians.ray_tracer is None:
        # The texture stage uses ray-traced visibility; extract a mesh now.
        extract_and_update_mesh(gaussExtractor, scene, dataset, opt, model_path, first_iter)
    ...
    while iteration < TOT_ITER:
        ...
        # Stage 4: switch from the baseline model to the textured model.
        if (not textured_stage) and iteration >= opt.texture_from_iter:
            print(f"\n[ITER {iteration}] Entering texture stage: transferring to TexturedGaussianModel.")
            gaussians = TexturedGaussianModel.from_baseline(gaussians, opt, opt.texture_size)
            gaussians.freeze_geometry()
            scene.gaussians = gaussians
            gaussExtractor.gaussians = gaussians
            textured_stage = True

        # Positions are frozen in the texture stage,
        # so the position LR schedule must not be advanced.
        if not textured_stage:
        ...
        with torch.no_grad():
            ...
            # Densification (disabled once the texture stage starts)
            if iteration < opt.densify_until_iter and iteration != opt.volume_render_until_iter and not textured_stage:
                ...
                if iteration % opacity_reset_intval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                    ...
                    gaussians.reset_metallic(exclusive_msk=outside_msk)
            ...
            if (iteration >= opt.indirect_from_iter and iteration % MESH_EXTRACT_INTERVAL == 0) or iteration == (opt.indirect_from_iter):
                if not HAS_RESET0:
                    extract_and_update_mesh(gaussExtractor, scene, dataset, opt, model_path, iteration)
...
def select_render_method(iteration, opt, initial_stage):
    ...
    elif iteration < opt.texture_from_iter:
        render = render_surfel
    else:
        render = render_surfel_textured

    return render


def extract_and_update_mesh(gaussExtractor, scene, dataset, opt, model_path, iteration):
    """Extract a mesh from the current Gaussians and hand it to the model's ray
    tracer (used for ray-traced indirect-light visibility)."""
    ...
    gaussExtractor.gaussians.update_mesh(mesh)
...
def reset_gaussian_para(gaussians, opt):
    gaussians.reset_albedo()
    gaussians.reset_metallic_value(opt.init_metallic_value)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10000,20000,30000,60000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[10000,20000,30000,60000])
    ...
    if not args.model_path:
        ...
        exp_name = "TextureSplat"
        save_path = os.path.join(
            "./output/", f"{last_subdir}/",
            f"{last_subdir}-{exp_name}"
        )
        # if the path already exists, add a timestamp
        if os.path.exists(save_path):
            current_time = datetime.now().strftime('%m%d_%H%M')
            # create a new path with the timestamp
            save_path = os.path.join(
                "./output/", f"{last_subdir}/",
                f"{last_subdir}-{exp_name}-{current_time}"
            )
