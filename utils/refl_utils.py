...
def get_full_color_surfel(envmap: torch.Tensor, albedo, HWK, R, T, normal_map, render_alpha, scaling_modifier = 1.0, metallic = None, roughness = None, pc=None, surf_depth=None, indirect_light=None):
    """
    Computes the full surface color including both diffuse and specular components.
    
    Args:
        envmap: Environment map function for lighting
        albedo: Surface albedo in (H,W,C) format
        HWK, R, T: Camera parameters
        normal_map: Surface normals in (H,W,C) format
        render_alpha: Alpha/opacity in (H,W,C) format
        scaling_modifier: Scale factor for output
        metallic: Metalness in [0,1], (H,W,C) format
        roughness: Surface roughness in (H,W,C) format
        pc: Point cloud for ray tracing (optional)
        surf_depth: Surface depth for ray tracing (optional)
        indirect_light: Indirect lighting contribution (optional)
        
    Returns:
        diffuse: Diffuse component in (C,H,W) format
        specular: Specular component in (C,H,W) format
        final_image: Combined diffuse and specular in (C,H,W) format
        extra_dict: Additional data for visualization/analysis
    """
    # Get specular component (returns in C,H,W format)
    if pc is not None and indirect_light is not None:
        specular, extra_dict = get_specular_color_surfel(envmap, albedo, HWK, R, T, normal_map, render_alpha, 
                                                         scaling_modifier, metallic, roughness, 
                                                         pc, surf_depth, indirect_light)
    else:
        specular, extra_dict = get_specular_color_surfel(envmap, albedo, HWK, R, T, normal_map, render_alpha,
                                                         scaling_modifier, metallic, roughness, 
                                                         pc, surf_depth)
    
    # Calculate diffuse component (all inputs are in H,W,C format)
    diffuse_raw = envmap(normal_map, mode="diffuse") * (1-metallic) * albedo
    # Apply alpha and permute to C,H,W format
    diffuse = (diffuse_raw * render_alpha).permute(2, 0, 1)
    
    # Combine diffuse and specular for final image (both in C,H,W format)
    final_image = diffuse + specular
    
    return diffuse, specular, final_image, extra_dict
