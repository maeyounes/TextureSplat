...
// Jacobian of L2-normalization for a 3-vector stored as a plain float array.
// Used when backpropagating through the normalization of texture-sampled
// normals. A small epsilon keeps the gradient finite for near-zero vectors.
__forceinline__ __device__ void dnormvdv(float* v, float* dv, float* dnormvdv)
{
	float sum2 = v[0] * v[0] + v[1] * v[1] + v[2] * v[2];
	float invsum32 = 1.0f / sqrt(sum2 * sum2 * sum2 + 1e-24f);

	dnormvdv[0] = ((+sum2 - v[0] * v[0]) * dv[0] - v[1] * v[0] * dv[1] - v[2] * v[0] * dv[2]) * invsum32;
	dnormvdv[1] = (-v[0] * v[1] * dv[0] + (sum2 - v[1] * v[1]) * dv[1] - v[2] * v[1] * dv[2]) * invsum32;
	dnormvdv[2] = (-v[0] * v[2] * dv[0] - v[1] * v[2] * dv[1] + (sum2 - v[2] * v[2]) * dv[2]) * invsum32;
}
