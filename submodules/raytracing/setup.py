...
def find_eigen(min_ver=(3, 3, 0)):
    ...
    import re, os
    try_paths = [
        # Look inside the active env first (e.g. `conda install -c conda-forge eigen`
        # installs headers at $CONDA_PREFIX/include/eigen3).
    ...
    if eigen_path is None:
        try:
            ...
            # Always download into _src_path so the include is an absolute path
            # — pip's PEP 517 builds change cwd, breaking any relative path.
            tmp_file = os.path.join(_src_path, TMP_EIGEN_FILE)
            tmp_dir  = os.path.join(_src_path, TMP_EIGEN_DIR)
