...
class EnvLight(torch.nn.Module):
    ...
    def __call__(self, l, mode=None, roughness=None):
        ...
        # nvdiffrast requires contiguous inputs; depending on the upstream
        # tensor layout (e.g. textured-stage permuted feature buffers) `l` /
        # `roughness` may be non-contiguous. .contiguous() is a layout-only
        # no-op and does not change rendered values.
        l = l.contiguous()
        if roughness is not None:
        ...
        if mode == "diffuse":
            # Diffuse lighting
            light = dr.texture(self.diffuse[None, ...].contiguous(), l, filter_mode='linear', boundary_mode='cube')
        elif mode == "pure_env":
            # Pure environment light (no mip-map)
            light = dr.texture(self.base[None, ...].contiguous(), l, filter_mode='linear', boundary_mode='cube')
        else:
            # Specular lighting with mip-mapping
            miplevel = self.get_mip(roughness)
            light = dr.texture(
                self.specular[0][None, ...].contiguous(),
                l,
                mip=list(m[None, ...].contiguous() for m in self.specular[1:]),
                mip_level_bias=miplevel[..., 0].contiguous(),
